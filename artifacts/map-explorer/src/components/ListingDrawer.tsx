                  ...
                  >
                    ...
                    <div style={{ padding: "8px 12px 10px", display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 8 }}>
                      <div style={{ minWidth: 0 }}>
                        ...
                        {loc.type === "school" && (() => {
                          const rank = (loc.school as any)?.metrics?.rankHistory?.[0];
                          if (!rank) return null;
                          const stars = Math.max(0, Math.min(5, Math.round(rank.rankStars ?? 0)));
                          return (
                            <div style={{ marginTop: 5, display: "flex", flexDirection: "column", gap: 2 }}>
                              <div style={{ display: "flex", gap: 1 }}>
                                {Array.from({ length: 5 }).map((_, i) => (
                                  <svg key={i} width={11} height={11} viewBox="0 0 24 24" fill={i < stars ? "#FBBF24" : "none"} stroke={i < stars ? "#FBBF24" : "#52525b"} strokeWidth={2}>
                                    <polygon points="12,2 15.09,8.26 22,9.27 17,14.14 18.18,21.02 12,17.77 5.82,21.02 7,14.14 2,9.27 8.91,8.26" />
                                  </svg>
                                ))}
                              </div>
                              <div style={{ fontSize: 10, color: "#71717a" }}>
                                #{rank.rank.toLocaleString()} of {rank.rankOf.toLocaleString()} {rank.rankLevel} schools in state · {rank.year}
                              </div>
                            </div>
                          );
                        })()}
